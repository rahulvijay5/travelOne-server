...
const initializeQueueSystem = async () => {
  ...
  try {
    ...
    connection.on('error', (error) => console.error('❌ Redis connection error:', error));
      ...
      async (job: Job) => {
        ...
        try {
          ...
          const booking = await prisma.booking.findUnique({
            where: { id: bookingId },
            include: { room: true, payment: true, hotel: { include: { managers: true } }, customer: true },
          });
          ...
          await prisma.$transaction(async (tx) => {
            ...
            await tx.room.update({ where: { id: booking.roomId }, data: { roomStatus: RoomStatus.AVAILABLE } });
            if (booking.payment) {
    ...
    cancellationWorker.on('error', (err) => {
      ...
      if (err.message.includes('timed out')) {
        console.debug('ℹ️ Redis command timed out (expected during idle periods)');
      } else {
    ...
    cancellationWorker.on('drained', () => {
      ...
      console.log(`ℹ️ Queue is empty, worker waiting for new jobs... Next check in ${minutes} minutes`);
    });
